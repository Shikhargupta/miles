...
def execute(
    ...
    *,
    ...
    assert_every_rank_trained_with_its_own_policy_args(events_dir, megatron_config=megatron_config)
    assert_every_policy_learned(events_dir, bounds=train_reward_bounds or TRAIN_REWARD_BOUNDS)


def assert_every_rank_trained_with_its_own_policy_args(events_dir: Path, *, megatron_config: dict) -> None:
    ...
    for model_id, ranks in ranks_by_model_id.items():
        assert ranks == every_rank, (
            f"policy {model_id!r} is reported from ranks {ranks}, while this run's trainer ranks are "
            f"{every_rank}; a rank that reported nothing leaves the arguments that rank was built with "
            f"unverified, however many times the other ranks reported"
        )
