...
def _build_script_args(
    ...
    assert not mode.colocate, (
        f"{TEST_NAME} deploys the trainer and the engines separately, which a colocated mode contradicts by "
        f"sharing gpus between them"
    )
...
app, run_ci = create_split_comparison_app_and_run_ci(
    test_name=TEST_NAME,
    mode=_MODE,
    build_baseline_args=_build_baseline_args,
    build_target_args=_build_args,
    build_deployments=_build_deployments,
    compare_fn=_compare,
)
