...
def create_split_target_run_side(*, build_deployments: BuildDeploymentsFn) -> RunSideFn:
...
def run_split_training(
    *, deployments: list[RunDeployment], launch: LaunchDeploymentFn, config: command_utils.ExecuteTrainConfig
    ...
    assert config.namespace, (
        "the deployments of one run reach each other by the in-cluster names of the releases that carry them, "
        "and a namespace is half of every such name"
    )
    assert config.run_uuid is not None, (
        f"the deployments of run {config.run_id} are installed by launches of their own and are joined by nothing "
        f"but the run uuid, so whatever installs them all has to name it"
    )
...
def _assert_deployments_form_one_run(deployments: list[RunDeployment]) -> None:
    ...
    assert len(drivers) == 1, (
        f"a run is driven by exactly one deployment carrying the orchestration script, and these deployments "
        f"carry it {len(drivers)} time(s): {[one.deploy_component.value for one in deployments]}"
    )
    assert deployments[-1] is drivers[0], (
        f"installing the deployment that carries the orchestration script blocks until the run finishes, so it "
        f"goes last; here it is {deployments.index(drivers[0])} of {len(deployments)}"
    )
...
def _assert_release_installed(*, release: str, namespace: str) -> None:
    assert Helm.get_manifest(release, namespace) is not None, (
        f"installing {release} returned without an error, but helm does not know it in namespace {namespace}, so "
        f"the run would wait forever for workers that nothing carries"
    )
