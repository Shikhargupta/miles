...
class PhaseHistory:
    ...
    def cells_that_healed_through_pending(self, *, cell_type: str | None = None) -> list[str]:
        return [
            name
            for name, phases in self.phases_of_cell_name.items()
            if (cell_type is None or self._cell_type_of_name[name] == cell_type) and _healed_through_pending(phases)
        ]
