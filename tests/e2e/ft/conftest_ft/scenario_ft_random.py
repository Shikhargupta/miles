...
def assert_rollout_healed_through_pending(injector: FaultInjectorHandle) -> None:
    ...
    healed = history.cells_that_healed_through_pending(cell_type="rollout")

    assert healed, (
        f"No rollout cell was observed going Running -> Pending -> Running after "
        f"{injector.num_successful_injections} successful injection(s) "
        f"(observed phases: {history.phases_of_cell_name})"
    )

    print(f"Rollout healing witness assertion passed: {healed} came back through Pending")
