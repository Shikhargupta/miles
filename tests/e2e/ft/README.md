...
Architecture (external fault injection, not inside training loop):
  ...
  8. Rollout healing witness: a rollout cell must be seen going Running -> Pending -> Running,
     proving its engine was replaced and came back serving. Suspended is not required in between —
     it lasts only --mini-ft-controller-resume-delay (10s), which a 2s poll can miss. Pending alone
     does not prove the replacement was gated: a disaggregated relaunch also passes through it.
