...
- **Entry file content**: `register_cuda_ci(est_time=..., suite=..., labels=[...])` plus `run_ci(_MODE)` under `__main__`, or `run_ci()` where the scenario takes no mode, no test logic.
...
- **Declared per mode**: cell count, parallelism, model, train/rollout GPU split, `colocate` (default disaggregated, i.e. training and rollout hold their own gpus), `ft_components` (default `("train",)`).
...
| --- | --- | --- |
| `run` | comparisons: prepare + every phase's baseline/target + compare. Soaks: one injected run plus its witnesses | all scenarios |
...
- **Dumps**: `/node_public/dumps/<test_name>/` via `resolve_dump_dir` in `conftest_ft/app.py`, deleted at the end of a comparison `run`; the soaks keep theirs.
...
- **One config per side**: each side builds its own `ExecuteTrainConfig` and threads it through `run_training()` and `api_server_host()`; on kubernetes the api server lives on a pod named after its `run_id`, so a second config would aim the injector at a release that does not exist.
...
| Backend | Cell type | Forms, each drawn once per cycle |
| --- | --- | --- |
...
| kubernetes | rollout | `delete_pod`, `exec_sigkill` |
...
- **Why a kubernetes engine takes no `inject_fault`**: its pod runs sglang as the entrypoint (`CommandWorkerSpec`), so no actor and no rpc server exist to receive one. `exec_sigkill` kills one of sglang's own subprocesses through `kubectl exec` instead, leaving the pod where it is; `delete_pod` reschedules. Only both cover the two recovery paths.
- **A form that was drawn and never once succeeded fails the soak**: a pod deletion refused all run is otherwise invisible, since the kills alone clear the injection floor.
...
Injection (target side only):
  ...
  2. Armed only once the target has finished a rollout, so no crash can land before generation
     has ever run
  3. Forms drawn per (cluster backend, cell type), as in the soaks
  4. Stop the injector, then re-use the soak's rollout witnesses: >= 2 accepted rollout
     injections, each paired with one completed recovery cycle
  5. Those injections must fall into at least two different generation windows
...
Assertions:
  ...
  6. Gradients were non-zero, per side: train/grad_norm is finite and non-zero in >= 2 rollouts,
     with --weight-decay 0 so nothing else can move the weights
```
...
- **Why weight decay is off and grad norms are checked**: the shared recipe carries `--weight-decay 0.1`, which moves every weight every step, so the weights-moved gate alone would pass a run whose gradients were all zero.
- **Why the injector waits for a rollout**: it schedules on wall clock, and start-up outlasts the first deadline, so both crashes can otherwise land before any generation ever runs.
...
Differs from the twin: train_async.py instead of train.py, plus --fully-async
                       --pause-generation-mode in_place and MILES_EXPERIMENTAL_ROLLOUT_REFACTOR=1;
                       test name gains a _fully_async suffix, which separates the dump dirs and
                       wandb runs
