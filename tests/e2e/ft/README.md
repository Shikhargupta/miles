...
- **A weight-update retry opens a new window**: only a failed trainer broadcast followed by a confirmed inference-controller close is retryable. A start, readiness, close, or drain failure stops instead of risking a second window while the first lock is ambiguous. Before a retry opens its fresh window, it waits one mini-controller poll interval, one live trainer cell, and the expected engine cell count while healing and reconcile stop or replace failed workers; the surviving trainer then takes a new engine snapshot instead of broadcasting against the previous snapshot while holding that window open.
