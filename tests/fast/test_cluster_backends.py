...
class TestKubernetesAvailability:
    ...
    def test_reports_a_timed_out_kubectl_as_unavailable_instead_of_raising(self, monkeypatch):
        """A black-holed apiserver must produce a skip with a reason, not a TimeoutExpired out of a probe."""

        def time_out(argv):
            raise subprocess.TimeoutExpired(cmd=argv, timeout=cluster_backends.KUBECTL_TIMEOUT_SECONDS)

        _fake_kubectl(monkeypatch, time_out)

        availability = cluster_backends.kubernetes_availability()

        assert not availability.available
        assert "60" in availability.reason and "mine" in availability.reason

    def test_reports_a_kubectl_that_cannot_be_executed_as_unavailable(self, monkeypatch):
        """The binary can vanish between the which() check and the call, and that is still just an unusable backend."""

        def fail_to_start(argv):
            raise OSError("Exec format error")

        _fake_kubectl(monkeypatch, fail_to_start)

        availability = cluster_backends.kubernetes_availability()

        assert not availability.available
        assert "Exec format error" in availability.reason
...
class TestCreateBackendForRun:
    ...
    def test_a_reachable_backend_hands_back_the_backend_the_run_asked_for(self, monkeypatch):
        """The gate must be invisible on the path the run is meant to take, and the caller runs on what it returns."""
        monkeypatch.setattr(
            cluster_backends,
            "availability_of_run",
            lambda *, config: cluster_backends.BackendAvailability(True, "fine"),
        )
        config = _run_config(ClusterBackend.RAY)

        backend = cluster_backends.create_backend_for_run(config)

        assert isinstance(backend, type(config.create_backend()))
