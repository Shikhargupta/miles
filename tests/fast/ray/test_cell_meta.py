...
def engine_spec(*, num_cells: int = 2, gpu_offset: int = 0) -> CommandWorkerSpec:
    ...
    return CommandWorkerSpec(
        ...
        meta=lambda context: dict(
            ...
            gpu_offset=gpu_offset + context.cell_index * GPUS_PER_NODE,
            sglang_api_key=None,
            needs_offload=False,
            update_weights=True,
        ),
...
def observed_meta(*, spec_name: str, cell_index: int, engine_cells: int = 2, trainer_cells: int = 1) -> dict[str, Any]:
    specs = specs_of(engine_cells=engine_cells, trainer_cells=trainer_cells)
    values = build_values(specs, LAYOUT)
    entry = next(
        candidate
        for section in ("inferenceEngines", "trainers")
        for candidate in values["run"][section]
        if candidate["specName"] == spec_name
    )
    ...
    factory = wiring.install_kubernetes_workers(
        ...
        delete_pods=_refuse_deletion,
    )
    provider = factory.cells(spec_names=[spec_name])

    async def scenario() -> dict[str, Any]:
        await provider.start()
        try:
            info = provider.cell_info(f"{spec_name}-{cell_index}")
...
async def _refuse_deletion(pod_names: list[str]) -> None:
    raise AssertionError(f"nothing in this test heals a cell, but {pod_names} were deleted")
...
def trainer_env_without_a_gpu(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(specs_train, "create_tcp_store", lambda: (object(), "127.0.0.1:1"))
...
class TestWhatAnEngineCellReports:
    def test_carries_every_fact_the_inference_controller_indexes(self):
        ...
        meta = observed_meta(spec_name=ENGINE_SPEC_NAME, cell_index=0)

        assert indexed_meta_keys(INFERENCE_CONTROLLER, attribute="meta") <= set(meta)

    def test_gives_each_cell_its_own_gpu_offset(self):
        """Two cells of one fleet share a values entry, and a shared offset would update one engine twice."""
        first = observed_meta(spec_name=ENGINE_SPEC_NAME, cell_index=0)
        second = observed_meta(spec_name=ENGINE_SPEC_NAME, cell_index=1)
    ...
    def test_keeps_the_types_the_consumers_expect(self):
        ...
        meta = observed_meta(spec_name=ENGINE_SPEC_NAME, cell_index=1)
    ...
    def test_still_reports_the_gpus_the_values_file_annotated(self):
        ...
        assert meta[GPU_IDS_META] == ",".join(str(gpu_id) for gpu_id in range(GPUS_PER_NODE))
...
class TestWhatATrainerCellReports:
    def test_carries_every_fact_the_trainer_group_indexes(self):
        ...
        meta = observed_meta(spec_name=TRAINER_SPEC_NAME, cell_index=0)

        assert indexed_meta_keys(TRAIN_GROUP, attribute="meta") <= set(meta)

    def test_reports_the_index_of_the_cell_that_was_observed(self):
        """Cells of one indep-dp fleet differ only by this index, so a constant would collapse them."""
        first = observed_meta(spec_name=TRAINER_SPEC_NAME, cell_index=0, trainer_cells=2)
        second = observed_meta(spec_name=TRAINER_SPEC_NAME, cell_index=1, trainer_cells=2)

        assert (first["cell_index"], second["cell_index"]) == (0, 1)
        assert isinstance(second["cell_index"], int)

    def test_reports_the_role_the_spec_was_built_for(self):
        """A critic fleet observes the same way, and the role is how a consumer tells the two apart."""
        assert observed_meta(spec_name=TRAINER_SPEC_NAME, cell_index=0)["role"] == "actor"
