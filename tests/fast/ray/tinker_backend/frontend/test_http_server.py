...
def make_app(api_key=API_KEY, ready=True):
    ...
    if ready:
        FakeDriver(backend)  # constructing the (fake) trainer flips readiness
...
class TestGuard:
    ...
    def test_operator_plane_is_loopback_only_even_with_the_sdk_key(self):
        # The SDK credential must never reach /adapter_runs (server-local
        # yaml_path reads, arbitrary save paths, deregister) from a remote peer.
        app = make_app()
        ...
        # ...and the key still applies on loopback.
...
class TestLaunchFlags:
    ...
    def test_plain_run_still_validates(self):
        validate_tinker_args(self.args())  # no tinker flags: nothing to check
