...
async def _fake_get_train_parallel_config(self: TrainerController) -> dict:
    return {"dp_size": 2 if self._role == "actor" else 99}


def _patch_train_group(monkeypatch) -> None:
...
async def _create_training_models(args, provider, rollout_executor):
    with patch("miles.utils.workers.worker_provider.ray.RayWorkerProvider.create", lambda: provider):
        return await placement_group_module.create_training_models(
            args,
...
async def test_critic_role_disables_reward_kl_and_preserves_actor_args(monkeypatch):
    ...
    actor, critic = await _create_training_models(args, provider, _RecordingRolloutExecutor())
...
async def test_train_parallel_config_travels_from_trainer_to_rollout_executor(monkeypatch):
    ...
    await _create_training_models(
        _training_models_args(use_critic=False), _RecordingWorkerProvider(), rollout_executor
    )
...
async def test_train_parallel_config_comes_from_the_actor_not_the_critic(monkeypatch):
    ...
    await _create_training_models(_training_models_args(use_critic=True), _RecordingWorkerProvider(), rollout_executor)
