...
class TestReconcile:
    ...
    @pytest.mark.asyncio
    async def test_a_cell_without_model_id_meta_is_ignored(self):
        """Non-engine cells (routers, session servers) carry no model_id and are skipped."""
        srv = _RecordingServer()
        controller = _make_controller({"default": srv})
        info = CellInfo(cell_id="miles-router-0-0", worker_names=["miles-router-0-0-0"], workers_hash="h", meta={})
