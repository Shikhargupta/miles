...
class TestCreateRolloutServersWiring:
    ...
    async def test_create_rollout_servers_wires_each_model_and_router_completely(self, tmp_path):
        """Every model is wired to its own injected router address, its own activeness tracker and cell count, and the injected lock, provider and update_weights survive."""
        cfg_path = tmp_path / "cfg.yaml"
        ...
        servers = await create_rollout_servers(
            ...
            router_addrs={
                "actor": HostAndPort(host="10.0.0.1", port=20000),
                "ref": HostAndPort(host="10.0.0.2", port=20001),
            },
        )
        ...
        assert {name: srv.expected_num_cells for name, srv in servers.items()} == {"actor": 2, "ref": 1}
        for srv in servers.values():
            assert srv.context_lock is lock
            assert srv.engine_provider is engine_provider
