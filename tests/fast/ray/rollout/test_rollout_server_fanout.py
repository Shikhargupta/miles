...
class _FailingMemoryCell(_RecordingCell):
    def __init__(self, *, cell_id: str, started: asyncio.Event, finish: asyncio.Event, error: BaseException):
        super().__init__(cell_id=cell_id, needs_offload=True)
...
class TestMemoryFanOutDuringReconcile:
    @pytest.mark.parametrize("op", ["offload", "onload"])
    async def test_a_replacement_can_reconcile_while_the_dead_generation_times_out(self, op: str):
        """A dead endpoint must not hold the controller lock while its replacement waits to reconcile."""
        started = asyncio.Event()
        finish = asyncio.Event()
        dead = _FailingMemoryCell(cell_id="cell", started=started, finish=finish, error=TimeoutError("dead endpoint"))
        replacement = _RecordingCell(cell_id="cell", needs_offload=True)
        srv = _make_server([dead])

        async def _transition() -> list:
            async with srv.context_lock:
                return await getattr(srv, op)(tags=["weights"])

        transition = asyncio.create_task(_transition())
        await started.wait()
        async with srv.context_lock:
            srv.server_cells["cell"] = replacement
        finish.set()
    ...
    @pytest.mark.parametrize("op", ["offload", "onload"])
    async def test_a_replacement_does_not_swallow_cancellation(self, op: str):
        """Replacing a generation must never turn controller cancellation into a successful transition."""
        started = asyncio.Event()
        finish = asyncio.Event()
        dead = _FailingMemoryCell(cell_id="cell", started=started, finish=finish, error=asyncio.CancelledError())
        replacement = _RecordingCell(cell_id="cell", needs_offload=True)
        srv = _make_server([dead])
