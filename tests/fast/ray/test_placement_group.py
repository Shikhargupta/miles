...
class TestCreateRolloutComponents:
    ...
    async def test_only_the_executor_is_started_here(self, fake_components):
        """The inference side is taken over after the trainers are drained, so nothing may touch it this early."""
        args = _make_args(num_rollout=1)

        await create_rollout_components(args)

        fake_components.executor_handle.init.assert_awaited_once_with()
        fake_components.controller_handle.is_initialized.assert_not_awaited()
...
class TestCreateTrainingModels:
    ...
    async def test_a_cold_run_asks_the_executor_for_no_rollout_state(self, tmp_path, monkeypatch):
        """Nothing was ever saved for rollout -1, so demanding it would refuse every fresh run."""
        ...
        rollout_executor.load.assert_awaited_once_with(-1, require_state=False)

    async def test_a_taken_over_run_demands_the_rollout_state_of_the_position_it_resumes_at(
        self, tmp_path, monkeypatch
    ):
        """The trainers already trained on those samples, so replaying the dataset from zero would repeat them."""
        self._patched(monkeypatch, [], initialized=True)
        rollout_executor = self._rollout_executor()

        await create_training_models(self._args(tmp_path), rollout_executor)

        rollout_executor.load.assert_awaited_once_with(-1, require_state=True)
