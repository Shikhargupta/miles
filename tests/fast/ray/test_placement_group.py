...
class TestCreatePlacementGroups:
    ...
    def test_each_role_views_the_shared_pg_from_its_own_offset(self, monkeypatch):
        """Roles share one placement group and see the reordered bundles and gpu ids from their own offset."""
        ...
        assert pgs["actor"].pg_reordered_gpu_ids == [100, 101, 102, 103, 104, 105]
        assert pgs["critic"].pg_reordered_gpu_ids == [102, 103, 104, 105]
        assert pgs["rollout"].pg_reordered_gpu_ids == [103, 104, 105]
        assert pgs["critic"].pg_reordered_bundle_indices == pgs["actor"].pg_reordered_bundle_indices[2:]
        assert pgs["rollout"].pg_reordered_bundle_indices == pgs["actor"].pg_reordered_bundle_indices[3:]
