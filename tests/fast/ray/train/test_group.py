...
class _FakeWorkerManager:
    ...
    def _get_cell_infos(self, *, spec_names: list[str]) -> dict:
        return {f"{spec_names[0]}-{index}": None for index in range(self.num_cells)}
...
class TestHeartbeatMonitor:
    ...
    async def test_the_group_activeness_flag_reaches_every_cell_checker(self):
        ...
        group._health_checker_activeness = False
        assert not any(c.health_checker._get_activeness() for c in group._cells)

        group._health_checker_activeness = True
        assert all(c.health_checker._get_activeness() for c in group._cells)
    ...
    async def test_the_paused_context_restores_activeness_after_an_exception(self):
        ...
        with pytest.raises(RuntimeError, match="boom"):
            with group._paused_health_checkers():
                assert not group._health_checker_activeness
