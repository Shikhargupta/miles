...
def kubernetes_availability_of_namespace(namespace: str, *, namespace_source: str) -> BackendAvailability:
    try:
        return _kubernetes_availability_of_namespace(namespace, namespace_source=namespace_source)
    except subprocess.TimeoutExpired as exc:
        command = shlex.join(exc.cmd) if isinstance(exc.cmd, list) else str(exc.cmd)
        return BackendAvailability(
            False, f"kubectl did not answer within {KUBECTL_TIMEOUT_SECONDS:g}s in namespace {namespace}: {command}"
        )
    except OSError as exc:
        return BackendAvailability(False, f"kubectl could not be run: {exc}")


def _kubernetes_availability_of_namespace(namespace: str, *, namespace_source: str) -> BackendAvailability:
