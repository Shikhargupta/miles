...
def test_example_launchers_only_emit_registered_miles_flags() -> None:
    ...
    unregistered: dict[str, set[Path]] = {
        flag: sources for flag, sources in emitted.items() if flag.startswith(MILES_OWNED_FLAG_PREFIXES) and flag not in registered
    }

    assert not unregistered, (
        "examples/ launchers pass flags that miles no longer registers: "
        + "; ".join(
...
def test_example_launchers_do_not_request_an_external_router() -> None:
    ...
    assert not offenders, (
        "external router mode was removed, so examples/ launchers must not pass: "
        + "; ".join(
            f"{flag} ({', '.join(sorted(str(p.relative_to(REPO_ROOT)) for p in sources))})"
