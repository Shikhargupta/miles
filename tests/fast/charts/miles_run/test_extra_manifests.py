...
from miles.utils.external_utils.command_utils.helm_backend.launcher.entrypoint import (
    _assert_extra_manifests_stay_in_namespace,
)
...
class TestNamespaceContract:
    def test_an_object_naming_another_namespace_is_refused(self):
        """The run uninstalls itself with a namespaced Role, so an object outside it survives and keeps its gpus."""
        manifest = "apiVersion: v1\nkind: Service\nmetadata:\n  name: extra\n  namespace: someone-else\n"

        with pytest.raises(AssertionError, match="own namespace"):
            _assert_extra_manifests_stay_in_namespace([manifest], namespace="mine")

    def test_an_object_with_no_namespace_is_accepted(self):
        """Helm installs it into the release namespace, which is exactly where it belongs."""
        _assert_extra_manifests_stay_in_namespace(
            ["apiVersion: v1\nkind: Service\nmetadata:\n  name: extra\n"], namespace="mine"
        )

    def test_an_object_naming_the_release_namespace_is_accepted(self):
        """Spelling out the namespace it would get anyway is not an error."""
        _assert_extra_manifests_stay_in_namespace(
            ["apiVersion: v1\nkind: Service\nmetadata:\n  name: extra\n  namespace: mine\n"], namespace="mine"
        )

    def test_every_document_of_a_multi_document_entry_is_checked(self):
        """One entry may hold several objects, and the second one must not slip past the first."""
        manifest = (
            "apiVersion: v1\nkind: Service\nmetadata:\n  name: a\n"
            "---\n"
            "apiVersion: v1\nkind: Service\nmetadata:\n  name: b\n  namespace: someone-else\n"
