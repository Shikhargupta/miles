...
ACTOR_AND_CRITIC = [
    ...
    {"name": "trainer-critic", "command": ["python", "-m", "miles.utils.workers.process_supervisor"]},
]


def _render(trainers=ACTOR_AND_CRITIC):
    return objects_of_kind(
        render_run("--set-json", f"run.trainers={json.dumps(with_object_names(trainers))}"), "LeaderWorkerSet"
    )
...
class TestTrainers:
    ...
    def test_gives_each_role_its_own_pool(self):
        ...
        assert [obj["metadata"]["name"] for obj in _render()] == [
            "myrun-miles-run-trainer-actor",
            "myrun-miles-run-trainer-critic",
        ]
