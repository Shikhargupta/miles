...
from tests.fast.charts.utils import CHART_DIR, CLI_PATH, cli_module
...
class TestInstall:
    def test_it_checks_then_vendors_then_upgrades(self, fake_tools):
        ...
        assert (
            f"helm upgrade --install wb {CHART_DIR} -n rl --set-string objectName=wb-miles-workbench --set-string infra.image.tag=v1"
            in calls
        )
...
class TestCli:
    ...
    def test_the_object_name_leaves_room_for_the_names_kubernetes_derives_from_it(self):
        ...
        name = cli_module().object_name("a" * 53)
    ...
    def test_the_object_name_carries_the_chart_name_only_once(self):
        ...
        assert cli_module().object_name("miles-workbench-alice") == "miles-workbench-alice"
        assert cli_module().object_name("alice") == "alice-miles-workbench"
