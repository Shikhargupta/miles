...
class TestReadCellIndex:
    def test_reads_the_group_out_of_the_fleet_leaders_address(self):
        """A pod is told its leader's hostname, and the group index is the only cell identity it carries."""
        assert read_cell_index({LEADER_ADDRESS_ENV_VAR: "myrun-miles-run-trainer-actor-3.myrun-miles-run"}) == 3
    ...
    def test_reads_a_bare_leader_hostname_too(self):
        ...
        assert read_cell_index({LEADER_ADDRESS_ENV_VAR: "trainer-actor-11"}) == 11

    def test_a_pod_outside_a_fleet_belongs_to_the_only_cell_there_is(self):
        """A static worker runs as a plain workload with one cell, so there is no group index to read."""
        assert read_cell_index({}) == 0

    def test_refuses_a_leader_address_with_no_group_index(self):
        """Guessing zero here would silently merge every cell of an indep-dp fleet into the first one."""
        with pytest.raises(AssertionError, match=LEADER_ADDRESS_ENV_VAR):
            read_cell_index({LEADER_ADDRESS_ENV_VAR: "leader.svc"})
...
class TestReadPodRank:
    ...
    def test_the_ctor_context_carries_the_cell_the_pod_is_in(self):
        ...
        context = rank.ctor_context(providers=FakeProviderFactory())
