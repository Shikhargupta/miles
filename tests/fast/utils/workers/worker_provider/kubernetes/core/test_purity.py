...
# A view may build an rpc handle, which drags in asyncio and httpx wherever it is imported from, so the
# invariant worth asserting is the cluster one: nothing a view reaches may speak to the apiserver.
FORBIDDEN_IMPORTS = (
    "kubernetes_asyncio",
...
def reachable_module_names(source_path: Path) -> list[str]:
    """Every module the view pulls in, following first-party imports as far as they go."""
    reached: list[str] = []
