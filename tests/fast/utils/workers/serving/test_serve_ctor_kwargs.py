...
class KeywordOnlyWorker:
    def __init__(self, *, args: str, rank: int, cell_index: int) -> None:
        self.args = args
        self.rank = rank
...
def ctor_kwargs(*, spec_name: str, worker_argv: list[str], context: WorkerCtorContext) -> dict[str, Any]:
    return dict(
        args=f"{spec_name}:{' '.join(worker_argv)}", rank=context.worker_in_cell_index, cell_index=context.cell_index
    )
...
def context_kwargs(*, spec_name: str, worker_argv: list[str], context: WorkerCtorContext) -> dict[str, Any]:
...
class TestCreateWorker:
    ...
    def test_the_context_carries_the_providers_the_spec_may_address_workers_through(
        ...
        assert worker.context.providers is not None
