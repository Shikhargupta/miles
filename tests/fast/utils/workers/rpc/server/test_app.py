...
class TestCapacity:
    ...
    async def test_oversized_chunk_is_rejected_before_copying_it(self) -> None:
        ...
        middleware = _RequestBodyLimitMiddleware(downstream, max_bytes=1024, boot_uuid="boot")
        tracemalloc.start()
...
class TestShutdown:
    ...
    async def test_concurrent_close_waiters_share_teardown_before_store_close(
        ...
        """Concurrent close callers cannot close retention before their shared executor teardown."""
        server = RpcServer(worker=_Worker())
        ...
        monkeypatch.setattr(server._executor, "close", close_executor)
        first_close = asyncio.create_task(server.close())
        await asyncio.wait_for(teardown_started.wait(), timeout=1.0)
        second_close = asyncio.create_task(server.close())
        first_close.cancel()
        with pytest.raises(asyncio.CancelledError):
            await first_close
        ...
        assert not server._store.closed
