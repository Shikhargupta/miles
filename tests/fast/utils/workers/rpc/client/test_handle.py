...
class TestWaitDead:
    ...
    async def test_wait_dead_gives_up_on_a_worker_that_stays_healthy(self, monkeypatch: pytest.MonkeyPatch):
        """A worker that refuses to die must not block healing forever, so the wait is bounded."""
        monkeypatch.setattr(rpc_handle_module, "_WAIT_DEAD_PROBE_INTERVAL_SECONDS", 0.01)
        async with _running_app(_Worker()) as app, _handle_over(httpx.ASGITransport(app=app)) as handle:
