...
def _make_serve_spec(name: str, *, port_infos: list[PortInfo] | None = None) -> ServeWorkerSpec:
    ...
    return ServeWorkerSpec(
        ...
        ctor_kwargs=lambda cell_index, worker_index: {"tag": "hello"},
    )
...
class TestInit:
    ...
    async def test_reserves_consecutive_port_blocks(self):
        """A dynamic port with num_consecutive reserves a whole block before the next dynamic port."""
        spec_name = _unique_name("serve")
        port_infos = [
            PortInfo(name="block", static_port=0, mode="per_worker", allow_dynamic=True, num_consecutive=5),
            PortInfo(name="single", static_port=0, mode="per_worker", allow_dynamic=True),
        ]
        manager = await _make_manager([_make_serve_spec(spec_name, port_infos=port_infos)])

        described = ray.get(ray.get_actor(f"{spec_name}-0-0").describe.remote())["addr_ports"]

        assert described["single_port"] == described["block_port"] + 5
        ray.kill(manager)
