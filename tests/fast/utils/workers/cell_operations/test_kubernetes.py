...
class TestWatching:
    def test_the_first_read_starts_the_watch_it_needs(self):
        ...
        operations = _operations({"trainer-actor-0": _info()})

        asyncio.run(operations.cell_infos(pool_ids=["trainer-actor"]))
    ...
    def test_later_reads_reuse_the_watch_already_running(self):
        ...
        async def scenario():
            await operations.cell_infos(pool_ids=["trainer-actor"])
            await operations.cell_infos(pool_ids=["trainer-actor"])
            await operations.suspend(cell_id="trainer-actor-0")
    ...
    def test_concurrent_first_reads_start_one_watch_between_them(self, deleted):
        ...
        operations = _operations({"trainer-actor-0": _info()}, start_delay=0.05)

        async def scenario():
            await asyncio.gather(*[operations.cell_infos(pool_ids=["trainer-actor"]) for _ in range(3)])
