...
class FakeHandle:
    def __init__(
        self, name: str, *, calls: list[tuple[str, str]], submitted: list[str], effect: str = "return"
    ) -> None:
        self._name = name
        self._calls = calls
    ...
    async def submit_without_result(self, method_name: str, /, **kwargs: str) -> None:
        self._submitted.append(method_name)
        self._calls.append((self._name, kwargs["mode"]))
...
class FakeProvider:
    def __init__(
        ...
        *,
        ...
        self.submitted_without_result: list[str] = []
    ...
    def get_handles_of_worker_infos(self, infos: list[WorkerInfo]) -> dict[str, FakeHandle]:
        return {
            info.name: FakeHandle(
                ...
                submitted=self.submitted_without_result,
...
class TestInjectFault:
    ...
    def test_returns_once_the_worker_accepted_the_call_rather_than_awaiting_its_result(self):
        """The callee dies mid-call, so waiting for a result burns the whole budget on a kill that already worked."""
        operations = _operations({"engine-0": _info(cell_id="engine-0", workers=("engine-0-0",))})

        asyncio.run(operations.inject_fault(cell_id="engine-0", mode=FailureMode.SIGKILL, sub_index=0))

        assert operations._provider.submitted_without_result == ["inject_fault"]
