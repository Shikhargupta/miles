...
from miles.utils.workers.ray_worker_manager import RayWorkerManager, bootstrapped_worker_class, build_serve_worker
...
def _make_spec(
    ...
    *,
    ...
    return ServeWorkerSpec(
        ...
        worker_class=worker_class if worker_class is not None else _WORKER_CLASS_PATH,
...
class TestTheHandleAWorkerIsCalledThrough:
    async def test_ray_communication_still_answers_with_a_ray_handle(self, fake_ray_cluster: FakeRayCluster):
        ...
        assert type(info.handle).__name__ == "RayWorkerHandle" and info.worker_class is None
    ...
    async def test_rpc_communication_answers_with_the_class_to_build_a_client_for(
        ...
        (info,) = manager.get_worker_infos("trainer-0")
        assert info.handle is None and info.worker_class == _WORKER_CLASS_PATH
    ...
    async def test_the_provider_turns_that_answer_into_an_rpc_handle(self, fake_ray_cluster: FakeRayCluster):
        ...
        (info,) = manager.get_worker_infos("trainer-0")
        handle = attach_rpc_handle(info).handle
    ...
    async def test_a_handle_that_is_already_built_is_left_alone(self, fake_ray_cluster: FakeRayCluster):
        """Command workers keep their ray handle under both modes, and rebuilding it would drop the actor."""
        manager = await _launch([_make_spec()])

        (info,) = manager.get_worker_infos("trainer-0")
        assert attach_rpc_handle(info).handle is info.handle
