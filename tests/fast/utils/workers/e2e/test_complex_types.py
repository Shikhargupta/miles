...
class TestComplexArgumentsAndResults:
    ...
    async def test_bytes_roundtrip(self, handle):
        """A bytes argument and result stay bytes across a text protocol."""
        result = await handle.demo_bytes(blob=b"raw-bytes")
    ...
    async def test_non_finite_float_result_is_rejected(self, handle):
        """json has no NaN, so the server sends null and the caller sees a validation error."""
        with pytest.raises(ValidationError):
            await handle.demo_nan_result()
