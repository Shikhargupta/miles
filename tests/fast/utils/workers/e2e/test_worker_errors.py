...
from miles.utils.workers.rpc.client.misc import NonRetryableRpcWorkerCallError, RpcWorkerCallError
...
class TestResultContract:
    ...
    async def test_result_type_mismatch_is_caught_client_side(self, handle):
        """A result that does not match the annotation already ran remotely, so retrying it would run it twice."""
        with pytest.raises(NonRetryableRpcWorkerCallError):
