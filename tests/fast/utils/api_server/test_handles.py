...
def _make_actor_handler(
    *,
    ...
    handler = _CellHandler(
        cell_type="actor", operations=RayCellOperations(manager), controller=group, spec_names=["trainer-actor"]
    )
...
def _make_rollout_handler(
    *,
    ...
    return _CellHandler(
        cell_type="rollout",
        operations=RayCellOperations(manager),
...
class TestRolloutCellHandler:
    ...
    async def test_a_suspended_cell_reports_no_health(self) -> None:
        ...
        handler = _make_rollout_handler(suspended=True, health=TriState.TRUE)
