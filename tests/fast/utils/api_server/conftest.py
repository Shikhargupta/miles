...
class MockWorkerManager:
    ...
    def get_cell_infos(self) -> MockRemoteCall:
        return MockRemoteCall(None, factory=self._summaries_of)

    def _summaries_of(self, *, spec_names: list[str]) -> dict[str, CellInfo]:
        return {k: v for k, v in self._summaries.items() if v.spec_name in spec_names}
