...
class TestRolloutCellHandle:
    ...
    async def test_get_cell_delegates_to_controller(self) -> None:
        ...
        handle = _RolloutCellHandle(inference_controller=controller, global_cell_id="actor--idx-0")
        cell = await handle.get_cell()

        assert cell.metadata.name == "rollout-actor--idx-0"
    ...
    async def test_suspend_delegates_to_controller(self) -> None:
        ...
        handle = _RolloutCellHandle(inference_controller=controller, global_cell_id="actor--idx-0")
        await handle.suspend()
        assert controller.stopped_cells == ["actor--idx-0"]
    ...
    async def test_resume_delegates_to_controller(self) -> None:
        ...
        handle = _RolloutCellHandle(inference_controller=controller, global_cell_id="actor--idx-0")
        await handle.resume()
        assert controller.started_cells == ["actor--idx-0"]

    def test_cell_type_is_rollout(self) -> None:
        handle = _RolloutCellHandle(inference_controller=object(), global_cell_id="actor--idx-0")
        assert handle.cell_type == "rollout"
        assert handle.cell_id == "rollout-actor--idx-0"
