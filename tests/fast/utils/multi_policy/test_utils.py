...
class TestCreatePolicyTrainers:
    ...
    async def test_the_executor_is_loaded_at_the_leader_policys_position(self, monkeypatch):
        ...
        rollout_executor.load.assert_awaited_once_with(3, require_state=False)
    ...
    async def test_a_taken_over_run_demands_the_rollout_state_of_the_leaders_position(self, monkeypatch):
        """Every policy resumed from a checkpoint, so replaying the shared dataset from zero would repeat samples."""
        ...
        rollout_executor.load.assert_awaited_once_with(3, require_state=True)
