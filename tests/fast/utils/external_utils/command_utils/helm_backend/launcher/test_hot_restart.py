...
_ARGV = ["python", "train.py", "--num-rollout", "16"]
_BOTH = list(HotRestartComponent)


def _wrapper_command(*, argv: list[str], state_file: str = "/runs/one/state.json") -> list[str]:
    return ["python", "-m", "miles.orchestrator.wrapper", "--state-file", state_file, "--", *argv]
                    ...
                    template=dict(
                        ...
                        spec=dict(
                            containers=[
                                dict(
                                    name=component,
                                    command=(_wrapper_command(argv=_ARGV) if command is None else command),
                                )
                            ]
                        ),
                    )
...
class TestTheStampAnOrdinaryRelaunchRenders:
    ...
    def test_a_relaunch_after_a_hot_restart_renders_the_stamp_it_finds_and_replaces_nothing(self):
        ...
        assert plan.restart_at == _STAMP_AT
        assert plan.stamped_components == {"orchestrator", "rollout-executor"}
        assert plan.allow_diff_object_keys == frozenset()
