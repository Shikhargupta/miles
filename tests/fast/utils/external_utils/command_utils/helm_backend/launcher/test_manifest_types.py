...
class TestFlagValue:
    def test_reads_what_the_installed_release_was_told(self):
        """An upgrade has to reuse the run uuid, and the pod command line is the only place it survives."""
        manifest = Manifest.parse(_rendered(_stateful_set(command=["python", "--run-uuid", "abc"])))

        assert manifest.flag_value("--run-uuid", stateful_set=ORCHESTRATOR, container="orchestrator") == "abc"
    ...
    def test_reads_only_the_object_it_was_asked_about(self):
        ...
        assert manifest.flag_value("--run-uuid", stateful_set="another-run", container="orchestrator") is None
    ...
    def test_reads_only_the_container_it_was_asked_about(self):
        ...
        assert manifest.flag_value("--run-uuid", stateful_set=ORCHESTRATOR, container="worker") is None
    ...
    def test_refuses_a_flag_the_command_leaves_unanswered(self):
        ...
        manifest = Manifest.parse(_rendered(_stateful_set(command=["python", "--run-uuid"])))
