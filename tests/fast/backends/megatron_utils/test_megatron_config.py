...
from miles.utils.arguments import capture_requested_checkpoint_source
...
class TestResolveMegatronConfig:
    ...
    def test_a_trainer_id_longer_than_the_pool_name_budget_is_refused(self, tmp_path):
        """An explicit trainer id bypasses the model id cap, so it needs a cap of its own."""
        path = _write_yaml(
            {"trainers": [{"model_id": "a", "trainer_id": "t" * (TRAINER_ID_MAX_LENGTH + 1)}]}, tmp_path
        )
...
class TestDerivedPerPolicyArgs:
    ...
    def test_a_model_id_longer_than_the_pool_name_budget_is_refused(self, tmp_path):
        """Pool names are length-capped, so a model id cannot consume the whole budget."""
        path = _write_yaml({"trainers": [{"model_id": "a" * (MODEL_ID_MAX_LENGTH + 1)}, {"model_id": "b"}]}, tmp_path)
...
class TestCoerceOverrideValue:
    def test_a_numeric_string_is_coerced_by_the_declared_type(self):
        """The coercion is what the declared type says it is, which needs no megatron parser to state."""
        coerced = _coerce_override_value("5e-7", key="lr", model_id="a", arg_types={"lr": float})

        assert coerced == 5e-7

    def test_a_whitelisted_argument_this_run_does_not_declare_is_refused(self):
        """A whitelist entry is not a promise that every backend's parser declares that argument."""
        with pytest.raises(AssertionError, match="declares no such argument"):
            _coerce_override_value(1e-8, key="min_lr", model_id="a", arg_types={})
