...
class TestNumGpusPerEnginePrecedence:
    ...
    def test_model_level_value_fills_groups_that_omit_it(self, tmp_path):
        """The model-level num_gpus_per_engine beats the args-level default for its groups."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: actor\n"
...
class TestOverridesResolution:
    def test_an_explicit_model_path_override_is_not_replaced(self, tmp_path):
        """A group's own model_path override survives resolve untouched."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: actor\n"
    ...
    def test_unrelated_override_keys_pass_through(self, tmp_path):
        """Arbitrary ServerArgs overrides survive resolve next to the injected model_path."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: actor\n"
        ...
        assert overrides["model_path"] == "/ckpt/actor"

    def test_conflicting_model_paths_across_groups_raise(self, tmp_path):
        """Two groups of one model resolving to different model_path values is rejected."""
        with pytest.raises(AssertionError, match="different model_path"):
            _resolve_yaml(
                tmp_path,
                "sglang:\n"
                "  - name: actor\n"
...
class TestNeedsOffloadFollowsTheColocatedFleet:
    ...
    def test_the_colocated_decode_fleet_offloads_even_though_prefill_is_declared_first(self, tmp_path):
        """Decode shares the trainer's gpus, so it is the fleet that has to make room for training."""
    ...
    def test_the_uncolocated_prefill_fleet_keeps_its_memory_and_disables_the_memory_saver(self, tmp_path):
    ...
    def test_naming_prefill_as_the_colocated_fleet_moves_the_offload_to_it(self, tmp_path):
        """The offload decision has to follow the declared pairing, whichever fleet that names."""
        prefill, decode = (
            self._resolve_pinned_pd(tmp_path, colocate_engine_spec="inference-engine-0-0").models[0].server_groups
        )
    ...
    def test_a_run_that_does_not_colocate_offloads_nothing(self, tmp_path):
        """Without colocation every fleet owns its gpus outright, so nothing has to yield them."""
    ...
    def test_ray_placement_still_decides_offload_by_the_gpu_ranges_it_hands_out(self, tmp_path):
        """Ray lays the whole rollout fleet over the trainer's bundles, so the gpu offsets are the truth there."""
