...
class TestPerPolicyQueues:
    async def test_a_group_of_two_policies_lands_in_a_queue_of_each(self):
        ...
        metrics = buffer.get_metrics()
        assert metrics["solver/rollout/fully_async/queue_size"] == 1
        assert metrics["verifier/rollout/fully_async/queue_size"] == 1
    ...
    async def test_every_policy_of_the_config_gets_a_queue_of_its_own(self):
        ...
        assert sorted({key.split("/")[0] for key in buffer.get_metrics()}) == ["solver", "verifier"]
