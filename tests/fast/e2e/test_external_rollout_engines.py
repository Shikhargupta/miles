...
class TestTheEnginesAreDescribedFromTheClusterItsOwnInfra:
    ...
    def test_an_image_name_that_would_break_the_yaml_is_quoted(self):
        """A values file is user text, and an unquoted name carrying a newline rewrites the container it belongs to."""
        infra = infra_with(image={**HOST_PATH_INFRA["image"], "tag": "dev\nprivileged: true"})

        assert container_of(infra)["image"] == "radixark/miles:dev\nprivileged: true"
        assert "privileged" not in pod_spec_of(infra)
