...
Grace-Blackwell NVLink-C2C link (~900 GB/s) makes a host read far cheaper than
the PCIe hop this pattern was originally designed around.

Out-of-range ids write zeros rather than reading, which is what makes the
tensor-parallel split work: every rank runs the gather over all ids, contributes
only rows it owns, and an all-reduce sums the pieces -- the same contract as
``VocabParallelEmbedding``.

No backward: the table is frozen (see the project notes on why full-parameter
training of it would cost 819 GB of optimizer state), so the gather is a leaf and
autograd never needs to walk back through it.
"""
...
def gather_ple_rows(
    ...
    ``host_table`` is the pinned CPU tensor holding rows ``[row_start, row_end)``.
    Ids outside that window come back as zeros.
    """
