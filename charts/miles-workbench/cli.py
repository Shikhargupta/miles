...
def doctor(args: argparse.Namespace) -> int:
    ...
    if args.rbac:
        ...
        elif checks.may_delegate_rules_it_does_not_hold(object_name(args.release)):
            checks.warn(
...
def exec_shell(args: argparse.Namespace) -> int:
    ...
    return run(
        ["kubectl", "exec", "-it", f"statefulset/{object_name(args.release)}", "-n", args.namespace, "--", *command]
    )
