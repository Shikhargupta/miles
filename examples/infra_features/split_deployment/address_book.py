...
class RunAddressBook:
    ...
    def of_config(cls, config: command_utils.ExecuteTrainConfig) -> "RunAddressBook":
        assert config.namespace, (
            "the deployments of one run reach each other by the in-cluster names of the releases that carry "
            "them, and a namespace is half of every such name"
        )
        assert config.run_uuid is not None, (
            "the deployments of one run are joined by nothing but the run uuid, and a split launch is never given "
            "one of its own, so every command that installs a part of this run has to carry the same --run-uuid"
        )
