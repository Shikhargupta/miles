...
on disjoint GPUs of one node concurrently. Uses train_async.py and the
...
Default split: 4 GPUs training (TP=4) + 2 GPUs inference (one TP=2 engine),
configurable via --train-num-gpus / --rollout-num-gpus; sized for a 6-GPU node.
...
Usage:
    python run-glm47-flash-agentic-async.py --num-gpus-per-node 6
    python run-glm47-flash-agentic-async.py --num-gpus-per-node 8 \\
        --train-num-gpus 4 --rollout-num-gpus 4
    python run-glm47-flash-agentic-async.py --num-gpus-per-node 6 \\
        --agent-server-url http://agent-server.example.com:11000 \\
        --router-external-host 10.0.0.5
"""
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    async_max_concurrent_samples: int = 8
    ...
    # Agent settings
    agent_server_url: str = os.environ.get("AGENT_SERVER_URL", "http://localhost:11000")
...
def execute(args: ScriptArgs):
    ...
    # Disaggregated split on one node: training and inference on disjoint GPUs.
    train_gpus = args.train_num_gpus
    ...
    # EP = largest divisor of 64 that also divides TP*DP (ETP=1 lets the
    # expert group span TP ranks, so experts are never replicated).
    tp, pp = 4, 1
    dp = train_gpus // (tp * pp)
    assert train_gpus % (tp * pp) == 0, f"train GPUs ({train_gpus}) must be divisible by TP*PP ({tp * pp})"
    num_experts = 64
    non_pp_gpus = tp * dp
    ep = max(d for d in range(1, non_pp_gpus + 1) if num_experts % d == 0 and non_pp_gpus % d == 0)
    ...
    # SGLang: one plain TP/EP engine over all rollout GPUs. Flash has 20
    # attention heads, so TP must divide 20 (2 and 4 are the realistic sizes).
    sglang_world_size = rollout_gpus
    assert 20 % sglang_world_size == 0, f"sglang TP ({sglang_world_size}) must divide 20 attention heads"
    print(f"Inference: 1 engine x {sglang_world_size} GPUs/engine")
    sglang_decode_max_bs = args.sglang_max_running_requests
