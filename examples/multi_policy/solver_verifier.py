...
_VERIFIER_PROMPT_TEMPLATE = (
    ...
    "Question:\n{question}\n\n"
    "Proposed solution:\n{solver_response}\n\n"
    f"Reason about the proposed solution, then end your reply with exactly one verdict word: {_AGREE_MARKER} "
    f"if the proposed final answer is correct, or {_WRONG_MARKER} if it is not. After a {_WRONG_MARKER} verdict, "
    "write your own final answer on a last line as '#### <answer>'."
)

_VERDICT_PATTERN = re.compile(rf"\b({_AGREE_MARKER}|{_WRONG_MARKER})\b")
...
def _parse_verdict(response: str) -> _Verdict | None:
    if not (found := _VERDICT_PATTERN.findall(response)):
        return None
    return _Verdict(found[-1])
