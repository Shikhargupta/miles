...
| `--colocate` | flag | off | Share GPUs between actor and rollout. Implicitly enables `--offload-train`, `--offload-rollout`, and defaults `--sglang-cuda-graph-backend-prefill=disabled`. Also valid with `--fully-async`; see [Fully Async RL](/user-guide/fully-async#colocate). |
