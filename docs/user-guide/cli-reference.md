...
|---|---|---|---|
| `--cluster-backend` | enum | `ray` | `ray` launches workers from the driver; `kubernetes` expects the platform to have created them already and observes them by their pod labels. `kubernetes` requires `--object-store-backend mooncake`. Under `kubernetes`, `--use-prometheus` is ignored. |
