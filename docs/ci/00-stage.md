...
**Docs-only skip.** `resolve-ci-policy` also emits `docs_only`, computed by `ci_policy.py` from the PR's changed-file list (merge commit vs `HEAD^1`, the `docker-paths` convention). It is `true` only when the trigger is a `pull_request`, the PR carries no CI label, the change list is non-empty, and every changed path sits under `docs/`; the GPU stages in `pr-test.yml` and the MI300X stage in `pr-test-rocm.yml` then skip. Every other case — scheduled and manual runs, any CI label, an empty or failed diff, any path outside `docs/` — resolves to `false` and runs the full fleet. The CPU stages ignore `docs_only` entirely, so the docs checks (docs.json navigation, examples-mirror drift) always run.
