...
The ROCm resolver uses only its dispatch input, defaults to its undated `rocm/sgl-dev` tag, and uses that default for called release runs too. A separate resolver decides whether the image's baked SGLang and Megatron-LM are kept or installed over.
