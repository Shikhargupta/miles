...
You need a kubectl context and an `infra.yaml` for the cluster; `charts/miles-run/values.yaml` shows
its shape. Everything testable without a cluster is tested
without one, so this runbook covers only what cannot be: real scheduling, real gpus, and CUDA IPC
between two pods.

Object names and cross-pool addresses are decided in Python, in
`miles/utils/external_utils/command_utils/helm_backend/naming.py`, and written into the values the
launcher generates. The chart computes no names of its own and copies what it is given, so `helm
template` over hand written values renders whatever names those values carry. Read a run's real
object names out of its generated values file rather than recomputing them.
