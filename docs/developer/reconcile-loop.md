...
| Upstream | Solves | Decision | Reason |
| --- | --- | --- | --- |
| Delaying queue on earliest `readyAt` | Rate-limited retry | **Replaced** by latest-wins deadlines swept by one `POLL_INTERVAL` loop | A new failure overwrites a deadline instead of cancelling a task, so the delay always matches current state. Go sweeps a heap on an exact timer inside one `waitingLoop` goroutine; polling keeps the single loop and costs a retry up to one interval late, which a backoff heuristic absorbs. The poller is created in `__init__` and cancelled in `shutdown()`, so its lifetime is a straight line; with nothing due it parks on an `asyncio.Event` rather than the clock, so an idle scheduler holds no wakeup. Creating it there is why a `ReconcileLoop` must be constructed inside a running event loop |
