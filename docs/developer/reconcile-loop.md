...
| Manual requeue | Re-drive a parent key nothing in the source changed | **`enqueue(parent_key)`** | A consumer that decides an object it derived from the store is dead cannot express that as a source event, because the platform's own view has not changed. Handing it the work queue keeps that decision level-triggered like every other one, and the reconcile it triggers still re-derives from `get_by_parent()` |
