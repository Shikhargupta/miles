...
class LinearTrajectory:
    ...
    def _try_detect_and_rollback_to_assistant_checkpoint(
        ...
        # Only responses generated by this session create checkpoints. Assistant
        # messages supplied by the client are ordinary prompt history.
        checkpoint_index = next(
            (
                i
                for i in range(len(self.generated_checkpoint_message_ends) - 1, -1, -1)
        ...
        if checkpoint_index < 0:
            raise MessageValidationError(
                f"rollback failed: no assistant checkpoint generated by the session found in the first "
                f"{match_len} matched messages (stored has {len(stored)} messages, "
                f"request has {len(request_messages)} messages)"
            )

        rollback_msg_end = self.generated_checkpoint_message_ends[checkpoint_index]
