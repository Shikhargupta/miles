...
class SessionCoreV2(SessionCore):
    ...
    def __init__(self, backend, registry: SessionRegistryV2, config, session_server_instance_id=None):
        super().__init__(backend, registry, config, session_server_instance_id)
    ...
    async def collect_samples(
        ...
        try:
            material = build_leaf_material(
                self.config, session, self.registry, session_id=session_id, max_seq_len=max_seq_len
            )
