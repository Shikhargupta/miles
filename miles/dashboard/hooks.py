...
def _collect_worker_infos(cells) -> list[list]:
    ...
    futures = []
    for cell in cells:
        spec_name, cell_index, _ = parse_worker_name(cell.meta.worker_name)
        futures.append(manager_handle.get_worker_infos.remote(spec_name=spec_name, cell_index=cell_index))
