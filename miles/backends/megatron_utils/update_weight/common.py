...
def begin_weight_update(rollout_engines: Sequence[SGLangApiClient]):
    """Open a weight-update session on all rollout engines (restore packed weights)."""
    async_utils.wait_futures([async_utils.submit(client.begin_weight_update()) for client in rollout_engines])
