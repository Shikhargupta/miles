...
from miles.utils.weight_version import generate_weight_version_run_uuid
...
def get_miles_extra_args_provider(add_custom_arguments=None):
    def add_miles_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                "--weight-version-run-uuid",
                type=str,
                default=None,
                help=(
                    "8-hex-char identifier embedded in every weight version string "
                    "('<run-uuid>-<rollout-id>') so samples generated by an engine of another "
                    "run are detected immediately. Auto-generated per launch when unset."
                ),
            )
            parser.add_argument(
...
def miles_validate_args(args):
    ...
    if args.weight_version_run_uuid is None:
        args.weight_version_run_uuid = generate_weight_version_run_uuid()
