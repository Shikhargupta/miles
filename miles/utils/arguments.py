        ...
        def add_cluster_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "splits the rollout into fleets: only the colocated fleet then shares the trainer's gpus, "
            ...
            parser.add_argument(
                "--colocate-engine-spec",
                type=str,
                default=None,
                help=(
                    "Name of the inference engine spec that shares the trainer's gpus, e.g. "
                    "inference-engine-0-1. Only needed when several engine fleets could be the colocated "
                    "one, e.g. when two weight-updated models each have a regular server group. With PD "
                    "disaggregation the decode fleet is the colocated one unless this names another."
                ),
            )
            parser.add_argument(
                "--offload",
...
def miles_validate_args(args):
    ...
    if args.colocate:
        ...
        if not declares_own_rollout_fleets and args.rollout_num_gpus != actor_num_gpus:
            logger.info(
