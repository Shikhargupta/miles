...
def miles_validate_args(args):
    ...
    if ClusterBackend(args.cluster_backend) == ClusterBackend.KUBERNETES:
        assert ObjectStoreBackend(args.object_store_backend) == ObjectStoreBackend.MOONCAKE, (
            f"--cluster-backend {ClusterBackend.KUBERNETES.value} needs "
            f"--object-store-backend {ObjectStoreBackend.MOONCAKE.value}: the ray store hands out references only "
            f"a ray driver can redeem, and kubernetes-native runs have no such driver"
        )
