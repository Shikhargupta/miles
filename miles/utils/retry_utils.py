...
async def _run_attempt(fn: Callable[[float], Awaitable[_T]], *, remaining: float, attempt_seconds: float | None) -> _T:
    ...
    try:
        return await asyncio.wait_for(fn(min(attempt_seconds, remaining)), timeout=attempt_seconds)
    except (TimeoutError, asyncio.TimeoutError) as e:
        raise AttemptTimeoutError(f"one attempt did not answer within {attempt_seconds}s") from e
