...
def assert_engine_count(*, side: str, dump_dir: str, expected: int) -> None:
    ...
    assert counted == [expected], (
        f"{side}: weights were pushed to {counted} engine(s), not {expected}; a run served by fewer engines than "
        f"it was given still trains, so nothing else here would notice engines that never joined it"
    )
