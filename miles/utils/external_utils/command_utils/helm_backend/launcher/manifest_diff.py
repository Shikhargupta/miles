...
class ManifestDiffs(FrozenStrictBaseModel):
    ...
    def rebuilds(self, *, key: ManifestObjectKey) -> bool:
        if any(identity.key == key for identity in (*self.added, *self.removed)):
            return True
...
def _compute_change(
    ...
    *,
    ...
    allowed_by = _compute_allowed_by(
        old.kind, identity=identity, path=path, allow_diff_object_keys=allow_diff_object_keys
    )
...
def _is_scaling(kind: str, path: tuple[str, ...]) -> bool:
    return kind in _SCALABLE_KINDS and path == _REPLICAS_PATH
