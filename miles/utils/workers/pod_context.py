...
class PodRank:
    ...
    def ctor_context(self, *, providers: ProviderFactory) -> WorkerCtorContext:
...
def read_pod_rank(*, ranks_per_pod: int, gpu_slots_per_rank: int, environ: Mapping[str, str] | None = None) -> PodRank:
    ...
    return PodRank(
        cell_index=read_cell_index(environ),
...
def read_cell_index(environ: Mapping[str, str]) -> int:
    ...
    assert separator and group_index.isdigit(), (
        f"{LEADER_ADDRESS_ENV_VAR} is {leader_address!r}, which does not end in the group index the fleet "
