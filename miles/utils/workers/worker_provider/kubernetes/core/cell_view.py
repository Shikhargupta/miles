...
def compute_cell_info(cell_id: str, *, pods: list[pod_view.ParsedPod], run: KubernetesRunInfo) -> CellInfo | None:
    ...
    return CellInfo(
        cell_id=cell_id,
        pool_id=pool_id,
        alive=all(pod.ready and not pod.deleting for pod in pods) and _has_every_pod(pods),
...
def compute_worker_infos(cell_id: str, *, pods: list[pod_view.ParsedPod], run: KubernetesRunInfo) -> list[WorkerInfo]:
    ...
    assert indices == list(range(len(pods))) and _has_every_pod(
        pods
...
def _compute_worker_info(worker: KubernetesWorkerInfo, *, run: KubernetesRunInfo) -> WorkerInfo:
    ...
    # a command worker (an engine, a router) declares no worker class, so nothing describes its rpc methods;
    # the dashboard still reads its name, addresses and gpus
    handle = (
