...
class CellLabelKeys(FrozenStrictBaseModel):
    """Where a pod publishes what it belongs to.

    LeaderWorkerSet publishes the group and worker index as labels but the group
    size as an annotation, so the two are named apart rather than read as one map.
    """
