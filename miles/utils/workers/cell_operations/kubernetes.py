...
class KubernetesCellOperations(BaseCellOperations):
    ...
    async def _ensure_watching(self) -> None:
        # the first call suspends until the apiserver answers its LIST, and the handlers of an api server
        # share one instance and are gathered concurrently, so every caller awaits the same start
