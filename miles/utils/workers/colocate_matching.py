...
def match_by_gpu(
    *, engine_placements: Iterable[GpuPlacement], trainer_placements: Iterable[GpuPlacement]
    ...
    assert not unmatched, (
        f"no trainer shares {_describe(unmatched)}; colocate needs a trainer worker on every engine gpu, "
    ...
    assert not duplicated, f"{_describe(duplicated)} is claimed by more than one engine worker"
...
def assert_layout_pairs(*, layout: PairingLayout) -> None:
    assert layout.pods_per_engine_cell <= layout.pods_per_trainer_cell, (
        ...
        f"{layout.pods_per_trainer_cell}; colocate needs every engine worker to sit on a trainer's node"
    )
    ...
    assert layout.engine_cells <= layout.trainer_cells * layout.engines_per_trainer_cell, (
        ...
        f"{layout.engines_per_trainer_cell} each; an engine worker whose gpu no trainer shares would "
