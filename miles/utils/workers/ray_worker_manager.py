...
class RayWorkerManager:
    ...
    async def _for_all_worker_managers(self, fn: Callable[["_BaseActorManager"], Any]):
        await asyncio.gather(*[fn(a) for g in self._group_infos.values() for c in g.cells for a in c.actors])
...
class _GroupManager:
    ...
    @classmethod
    def initial(cls, spec: BaseWorkerSpec, manager_ref: RayWorkerManager) -> "_GroupManager":
        return cls(
            spec=spec,
            cells=[
                _CellManager(
...
class _BaseActorManager(Generic[SpecT]):
    ...
    def master_mode_addrs(self) -> NamedHostAndPorts:
        return {
            info.name: self.self_addrs[info.name]
            for info in self.spec.port_infos
            if info.mode == "master"
        }
