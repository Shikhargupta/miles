...
from miles.ray.specs.inference import compute_router_pool, compute_session_server_instance_id, spec_session_server
from miles.utils.http_utils import find_available_port, is_port_available, wait_tcp_ready
...
async def wait_router_ready(model_idx: int) -> HostAndPort:
    ...
    worker_name = compute_worker_name(
        pool=compute_router_pool(model_idx), cell_index=0, worker_in_cell_index=0
    )
