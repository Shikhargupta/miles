...
from miles.ray.specs.inference import compute_router_spec_name, compute_session_server_instance_id, spec_session_server
...
async def wait_router_ready(model_idx: int) -> HostAndPort:
    ...
    worker_name = compute_worker_name(
        spec_name=compute_router_spec_name(model_idx), cell_index=0, worker_in_cell_index=0
    )
