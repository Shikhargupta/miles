...
from miles.ray.specs.inference import compute_router_pool, compute_session_server_instance_id
...
async def resolve_router_addrs(args) -> dict[str, HostAndPort]:
    ...
    """
    if args.sglang_router_ip is not None or args.sglang_router_port is not None:
...
async def wait_router_ready(model_idx: int) -> HostAndPort:
    ...
    worker_name = compute_worker_name(pool=compute_router_pool(model_idx))
...
async def wait_session_server_ready(args):
    ...
    provider: BaseWorkerProvider = RayWorkerProvider.create()  # TODO inject instance
    addrs = [
        (await provider.get_addrs(worker_name=compute_worker_name(pool="session-server", cell_index=i)))["primary"]
