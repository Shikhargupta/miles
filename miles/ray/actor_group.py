...
class RayTrainGroup:
    ...
    async def update_weights(self, rollout_id: int | None = None):
        ...
        if self.args.use_fault_tolerance:
            await self._inference_controller.recover_updatable_engines.remote()

        info = await self._inference_controller.get_updatable_engines_and_lock.remote()
        await self._inference_controller.health_monitoring_pause.remote()
    ...
    async def set_rollout_components(self):
        await self._broadcast("set_rollout_components", self._inference_controller, self._rollout_executor)
