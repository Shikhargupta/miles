...
from miles.utils.workers.ray_worker_manager import RAY_WORKER_MANAGER_ACTOR_NAME, RayWorkerManager, SpecPlacement
...
def launch_worker_manager(worker_specs: list[BaseWorkerSpec]) -> ray.actor.ActorHandle:
    ...
    manager = ray.remote(RayWorkerManager).options(name=RAY_WORKER_MANAGER_ACTOR_NAME, num_cpus=1).remote()
    ray.get(manager.init.remote(worker_specs=worker_specs, placements=placements))
    return manager


def get_worker_manager() -> ray.actor.ActorHandle:
    return ray.get_actor(RAY_WORKER_MANAGER_ACTOR_NAME)
...
def _create_placements(worker_specs: list[BaseWorkerSpec]) -> dict[str, SpecPlacement]:
    ...
    for spec in worker_specs:
        ...
        if scheduling.num_gpus_per_worker == 0:
            continue
        assert (
            scheduling.num_gpus_per_worker <= 1
