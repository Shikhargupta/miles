...
from miles.utils.workers.types import ClusterBackend, resolve_worker_comm_backend
...
def _launch_ray_worker_manager(args):
    ...
    comm_backend = resolve_worker_comm_backend(
        cluster_backend=ClusterBackend(args.cluster_backend), requested=args.worker_comm_backend
    )
    return RayWorkerManager.launch(args, specs, pgs, comm_backend=comm_backend)
