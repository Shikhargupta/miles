...
class RayTrainGroup:
        ...
        def _create_cell(cell_index: int):
            ...
            if health_checker_config is not None:
                cell.health_checker = create_trainer_cell_health_checker(
                    ...
                    get_activeness=lambda: self._health_checker_activeness,
                )
    ...
    def _compute_num_cells(self) -> int:
        return len(ray.get(self._worker_manager.get_cell_infos.remote(spec_names=[self._spec_name])))
    ...
    async def _gather_all_alive_and_catch(self, compute_coroutine, *, debug_name: str, check_recoverable: bool = True):
        ...
        if check_recoverable and not self._is_recoverable():
            raise self._make_all_cells_failed_error(f"All cells failed during {debug_name}") from _first_exception(
                outputs
            )
